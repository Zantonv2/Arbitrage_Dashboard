...
use arbitrage_server::key_store::{KeyStore, ExchangeCredentials, KeyStoreError};
...
fn test_keystore_with_master_key() {
    ...
    let keystore = KeyStore::with_master_key(master_key).expect("Failed to create keystore with master key");
    assert_eq!(keystore.list_exchanges().len(), 0);
}
...
fn test_store_and_retrieve_credentials() {
    ...
    // Store credentials
    keystore.store_credentials(credentials.clone()).expect("Failed to store credentials");
    
    // Retrieve credentials
    let retrieved = keystore.get_credentials(&ExchangeId::OKX).expect("Failed to retrieve credentials");
...
fn test_store_multiple_exchanges() {
    ...
    for exchange in &exchanges {
        ...
        keystore.store_credentials(credentials).expect("Failed to store credentials");
    }
...
fn test_remove_credentials() {
    ...
    keystore.remove_credentials(&ExchangeId::OKX).expect("Failed to remove credentials");
    assert!(!keystore.has_credentials(&ExchangeId::OKX));
...
fn test_enable_disable_credentials() {
    ...
    // Disable
    keystore.set_enabled(&ExchangeId::OKX, false).expect("Failed to disable");
    assert!(!keystore.get_enabled_exchanges().contains(&ExchangeId::OKX));
    
    // Re-enable
    keystore.set_enabled(&ExchangeId::OKX, true).expect("Failed to enable");
    assert!(keystore.get_enabled_exchanges().contains(&ExchangeId::OKX));
}
...
fn test_get_enabled_exchanges_only() {
    ...
    keystore.set_enabled(&ExchangeId::ByBit, false).expect("Failed to disable");
...
fn test_overwrite_existing_credentials() {
    ...
    keystore.store_credentials(credentials1).expect("Failed to store initial credentials");
    
    // Overwrite with new credentials
    let mut credentials2 = create_test_credentials(ExchangeId::OKX);
    credentials2.api_key = "updated_key".to_string();
    keystore.store_credentials(credentials2).expect("Failed to store updated credentials");
...
fn test_sandbox_flag() {
    ...
    keystore.store_credentials(prod_credentials).expect("Failed to store production credentials");
...
fn test_all_supported_exchanges() {
    ...
    for exchange in &all_exchanges {
        let credentials = create_test_credentials(*exchange);
        keystore.store_credentials(credentials).expect(&format!("Failed to store credentials for {}", exchange));
    }
    ...
    for exchange in all_exchanges {
        assert!(keystore.has_credentials(&exchange));
        let retrieved = keystore.get_credentials(&exchange).expect("Failed to retrieve credentials");
...
fn test_concurrent_access() {
    ...
    let keystore = Arc::new(Mutex::new(KeyStore::new().expect("Failed to create keystore")));
    ...
    for i in 0..5 {
        ...
        let handle = thread::spawn(move || {
            ...
            ks.store_credentials(credentials).expect("Failed to store credentials");
        });
        handles.push(handle);
    }
