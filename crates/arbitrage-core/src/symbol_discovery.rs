...
pub enum DiscoveryEvent {
    ...
    CriteriaUpdated { new_criteria: SymbolSelectionCriteria },
...
impl SymbolDiscoveryService {
    ...
    pub async fn update_market_data(&mut self, market_info: MarketInfo) -> Result<()> {
        ...
        if age_seconds > self.criteria.max_data_age_seconds {
            return Err(ArbitrageError::Validation(format!(
                ...
                age_seconds, self.criteria.max_data_age_seconds, exchange, symbol.to_pair()
            )));
        }
        ...
        if !was_qualified && is_qualified {
            ...
            let _ = self.event_sender.send(DiscoveryEvent::SymbolAdded {
                symbol: symbol.clone(),
                reason: format!("Meets criteria: volume=${}, spread={}bps", 
    ...
    pub fn get_arbitrage_symbols(&self) -> Result<Vec<Symbol>> {
        let mut scored_symbols: Vec<(Symbol, Decimal)> = self.symbol_scores
            .iter()
        ...
        Ok(scored_symbols.into_iter().map(|(symbol, _)| symbol).collect())
    }

    /// Calculate comprehensive symbol score
    async fn calculate_symbol_score(&mut self, symbol: &Symbol) -> Result<()> {
        let markets: Vec<&MarketInfo> = self.market_data
            .values()
        ...
        let avg_level1_liquidity: Decimal = markets.iter()
            .map(|m| m.depth_analysis.level_1_volume_usd)
            .sum::<Decimal>() / Decimal::from(markets.len());
        ...
        let spread_variance: f64 = spreads.iter()
            .map(|&s| (s as f64 - avg_spread).powi(2))
            .sum::<f64>() / spreads.len() as f64;
        let stability_score = Decimal::from(100) - Decimal::try_from(spread_variance.sqrt()).unwrap_or(Decimal::from(100));
        
        // Calculate volume score (relative to all symbols)
        let total_volume: Decimal = markets.iter().map(|m| m.volume_24h_usd).sum();
        let volume_score = (total_volume / Decimal::from(10_000_000) * Decimal::from(100))
            .min(Decimal::from(100));
        
        // Weighted total score
        let total_score = 
            arbitrage_potential * Decimal::new(4, 1) +  // 40% weight
    ...
    fn update_symbol_history(&mut self, symbol: &Symbol, market_info: &MarketInfo) {
        let history = self.symbol_history.entry(symbol.clone()).or_insert_with(Vec::new);
        
        // Calculate current arbitrage potential
        let current_arbitrage_potential = self.symbol_scores
            .get(symbol)
    ...
    pub fn get_market_stats(&self, symbol: &Symbol) -> Option<EnhancedSymbolStats> {
        ...
        let total_volume: Decimal = markets.iter().map(|m| m.volume_24h_usd).sum();
        let avg_spread_bps = markets.iter().map(|m| m.spread_bps as u64).sum::<u64>() / markets.len() as u64;
        
        // Enhanced liquidity metrics
        let total_level1_liquidity: Decimal = markets.iter()
            .map(|m| m.depth_analysis.level_1_volume_usd)
            .sum();
        let avg_depth_01: Decimal = markets.iter()
        ...
        Some(EnhancedSymbolStats {
            ...
            liquidity_metrics: LiquidityMetrics {
                ...
                liquidity_score: self.symbol_scores.get(symbol).map(|s| s.liquidity_score).unwrap_or(Decimal::ZERO),
            },
    ...
    fn calculate_trend(&self, symbol: &Symbol) -> Option<TrendAnalysis> {
        ...
        Some(TrendAnalysis {
            ...
            avg_arbitrage_potential: recent_points.iter()
                .map(|p| p.arbitrage_potential)
                .sum::<Decimal>() / Decimal::from(recent_points.len()),
    ...
    pub async fn update_criteria(&mut self, new_criteria: SymbolSelectionCriteria) -> Result<()> {
        ...
        let symbols: Vec<Symbol> = self.market_data.values()
        ...
        let _ = self.event_sender.send(DiscoveryEvent::CriteriaUpdated {
            new_criteria,
        });
