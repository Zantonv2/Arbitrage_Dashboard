use crate::strategies::{Strategy, MarketBundle, RawSignal, FilterContext, StrategyConfig, TradeLeg};
...
impl CrossExchangeArbitrageStrategy {
    ...
    pub fn new() -> Self {
        ...
        config.min_profit_bps = StrategyLimits::get_min_profit_bps("cex_arbitrage"); // Same as CEX arbitrage
        config.max_exposure = StrategyLimits::get_max_exposure("cex_arbitrage");
        
        // Add cross-exchange specific parameters
        let mut custom_params = StrategyUtils::create_base_custom_params();
        custom_params.insert("min_balance_ratio".to_string(), json!(0.1)); // 10% minimum balance
        custom_params.insert("rebalance_threshold_pct".to_string(), json!(0.05)); // 5% rebalance threshold
        custom_params.insert("transfer_cost_bps".to_string(), json!(10)); // 0.1% transfer cost
        custom_params.insert("balance_optimization".to_string(), json!(true));
        custom_params.insert("max_imbalance_pct".to_string(), json!(0.3)); // 30% max imbalance
    ...
    fn find_best_bid(&self, market_data: &MarketBundle, symbol: &Symbol) -> Option<(ExchangeId, Decimal, Decimal)> {
        ...
        for exchange in self.get_supported_exchanges() {
            if let Some(order_book) = market_data.get_order_book(exchange, symbol) {
                if let Some(best_bid_level) = order_book.best_bid() {
                    match best_bid {
                        None => best_bid = Some((exchange, best_bid_level.price, best_bid_level.quantity)),
                        Some((_, current_price, _)) => {
                            if best_bid_level.price > current_price {
                                best_bid = Some((exchange, best_bid_level.price, best_bid_level.quantity));
                            }
                        }
                    }
                }
    ...
    fn find_best_ask(&self, market_data: &MarketBundle, symbol: &Symbol) -> Option<(ExchangeId, Decimal, Decimal)> {
        ...
        for exchange in self.get_supported_exchanges() {
            if let Some(order_book) = market_data.get_order_book(exchange, symbol) {
                if let Some(best_ask_level) = order_book.best_ask() {
                    match best_ask {
                        None => best_ask = Some((exchange, best_ask_level.price, best_ask_level.quantity)),
                        Some((_, current_price, _)) => {
                            if best_ask_level.price < current_price {
                                best_ask = Some((exchange, best_ask_level.price, best_ask_level.quantity));
                            }
                        }
                    }
                }
    ...
    fn calculate_gross_profit_bps(&self, buy_price: Decimal, sell_price: Decimal) -> Result<i32> {
        if buy_price.is_zero() {
            return Err(ArbitrageError::Calculation("Buy price cannot be zero".to_string()));
        }
        
        let profit_ratio = (sell_price - buy_price).checked_div(buy_price)
            .ok_or_else(|| ArbitrageError::Calculation("Division by zero in profit calculation".to_string()))?;
            
        let profit_bps = profit_ratio.checked_mul(Decimal::from(10000))
            .ok_or_else(|| ArbitrageError::Calculation("Profit calculation overflow".to_string()))?
            .to_i32()
            .ok_or_else(|| ArbitrageError::Calculation("Profit BPS conversion failed".to_string()))?;
            
        Ok(profit_bps)
    }
    
    /// Estimate net profit after fees and transfer costs
    fn estimate_net_profit_bps(&self, gross_profit_bps: i32, buy_exchange: ExchangeId, 
                              sell_exchange: ExchangeId, needs_rebalancing: bool) -> i32 {
        ...
        if needs_rebalancing {
            let transfer_cost_bps = self.config.custom_params.get("transfer_cost_bps")
                .and_then(|v| v.as_i64())
                .unwrap_or(10) as i32;
    ...
    fn needs_rebalancing(&self, context: &FilterContext, buy_exchange: ExchangeId, 
                        sell_exchange: ExchangeId, symbol: &Symbol, quantity: Decimal, price: Decimal) -> bool {
        ...
        let min_balance_ratio = self.config.custom_params.get("min_balance_ratio")
            .and_then(|v| v.as_f64())
            .unwrap_or(0.1);
        ...
        if total_base > Decimal::ZERO {
            let sell_ratio = available_base.checked_div(total_base).unwrap_or(Decimal::ZERO);
        ...
        if total_quote > Decimal::ZERO {
            let buy_ratio = available_quote.checked_div(total_quote).unwrap_or(Decimal::ZERO);
    ...
    fn calculate_optimal_position_size(&self, context: &FilterContext, symbol: &Symbol,
                                     buy_exchange: ExchangeId, sell_exchange: ExchangeId,
                                     buy_price: Decimal, available_liquidity: Decimal) -> Decimal {
        let base_asset = &symbol.base;
        let quote_asset = &symbol.quote;
        ...
        let max_by_quote = available_quote.checked_div(buy_price).unwrap_or(Decimal::ZERO);
        let max_by_exposure = self.config.max_exposure.checked_div(buy_price).unwrap_or(Decimal::ZERO);
    ...
    fn is_balance_distribution_optimal(&self, context: &FilterContext, asset: &str) -> bool {
        let max_imbalance_pct = self.config.custom_params.get("max_imbalance_pct")
            .and_then(|v| v.as_f64())
            .unwrap_or(0.3);
        ...
        let ideal_balance_per_exchange = total_balance.checked_div(Decimal::try_from(num_exchanges).unwrap_or(Decimal::ONE))
            .unwrap_or(Decimal::ZERO);
        
        for exchange in self.get_supported_exchanges() {
            let balance = context.get_inventory(exchange, asset);
            let deviation = (balance - ideal_balance_per_exchange).abs();
            let deviation_ratio = deviation.checked_div(total_balance).unwrap_or(Decimal::ZERO);
...
impl Strategy for CrossExchangeArbitrageStrategy {
    ...
    fn detect(&self, market_data: &MarketBundle) -> Result<Vec<RawSignal>> {
        ...
        debug!("Cross-exchange arbitrage scanning {} symbols", symbols.len());
        ...
        for symbol in symbols {
            ...
            signal.add_metadata("available_liquidity", json!(available_liquidity.to_string()));
            
            signals.push(signal);
            
            debug!("Cross-exchange arbitrage signal: {} buy@{} {} sell@{} {} profit={}bps", 
                   symbol, buy_exchange, buy_price, sell_exchange, sell_price, gross_profit_bps);
        }
        
        debug!("Cross-exchange arbitrage detected {} signals", signals.len());
        Ok(signals)
    }
    ...
    fn filter(&self, signal: &RawSignal, context: &FilterContext) -> Result<bool> {
        ...
        let total_notional = optimal_size.checked_mul(buy_leg.price).unwrap_or(Decimal::ZERO);
        ...
        let balance_optimization = self.config.custom_params.get("balance_optimization")
        ...
        if balance_optimization {
            ...
            let base_distribution_optimal = self.is_balance_distribution_optimal(context, base_asset);
            let quote_distribution_optimal = self.is_balance_distribution_optimal(context, quote_asset);
