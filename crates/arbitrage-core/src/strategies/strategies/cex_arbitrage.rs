...
use crate::strategies::strategies_specifics::{CexArbitrageDefaults, ExchangeCapabilities, StrategyLimits};
...
impl CexArbitrageStrategy {
    ...
    pub fn new() -> Self {
        ...
        config.min_profit_bps = StrategyLimits::get_min_profit_bps("cex_arbitrage");
        config.max_exposure = StrategyLimits::get_max_exposure("cex_arbitrage");
        
        // Add CEX arbitrage specific parameters
        config.custom_params = CexArbitrageDefaults::get_custom_params();
    ...
    fn find_best_bid(&self, market_data: &MarketBundle, symbol: &Symbol) -> Option<(ExchangeId, Decimal)> {
    ...
    fn find_best_ask(&self, market_data: &MarketBundle, symbol: &Symbol) -> Option<(ExchangeId, Decimal)> {
    ...
    fn calculate_gross_profit_bps(&self, buy_price: Decimal, sell_price: Decimal) -> Result<i32> {
        ...
        let profit_bps = profit_decimal.to_i32()
            .ok_or_else(|| ArbitrageError::Calculation(
                format!("Profit calculation overflow: {}", profit_decimal)
            ))?;
    ...
    fn get_available_liquidity(&self, market_data: &MarketBundle, symbol: &Symbol, 
                              buy_exchange: ExchangeId, sell_exchange: ExchangeId) -> Result<Decimal> {
        let buy_book = market_data.get_order_book(buy_exchange, symbol)
            .ok_or_else(|| ArbitrageError::Validation("Buy order book not found".to_string()))?;
            
        let sell_book = market_data.get_order_book(sell_exchange, symbol)
            .ok_or_else(|| ArbitrageError::Validation("Sell order book not found".to_string()))?;
        
        // Get liquidity from both sides
        let buy_liquidity = buy_book.best_ask()
            .map(|level| level.quantity)
            .unwrap_or_else(|| Decimal::ZERO);
            
        let sell_liquidity = sell_book.best_bid()
            .map(|level| level.quantity)
            .unwrap_or_else(|| Decimal::ZERO);
    ...
    fn get_custom_param_u64(&self, key: &str, default: u64) -> u64 {
        self.config.custom_params.get(key)
            .and_then(|v| v.as_u64())
            .unwrap_or(default)
    }
    ...
    fn estimate_net_profit_bps(&self, gross_profit_bps: i32, buy_exchange: ExchangeId, sell_exchange: ExchangeId) -> i32 {
...
impl Strategy for CexArbitrageStrategy {
    ...
    fn detect(&self, market_data: &MarketBundle) -> Result<Vec<RawSignal>> {
        ...
        println!("CEX arbitrage scanning {} symbols: {:?}", symbols.len(), symbols);
        ...
        for symbol in symbols {
            ...
            println!("Arbitrage opportunity: Buy {} @ {} on {}, Sell {} @ {} on {}", 
            ...
            // Estimate net profit after fees
            let net_profit_bps = self.estimate_net_profit_bps(gross_profit_bps, buy_exchange, sell_exchange);
            println!("Net profit after fees: {} bps (min required: {} bps)", net_profit_bps, self.config.min_profit_bps);
            
            // Skip if net profit is too low
            if net_profit_bps < self.config.min_profit_bps {
                println!("Net profit too low: {}bps < {}bps", net_profit_bps, self.config.min_profit_bps);
                debug!("Net profit too low for {}: {}bps < {}bps", symbol, net_profit_bps, self.config.min_profit_bps);
                continue;
            }
            
            // Get available liquidity
            let liquidity = match self.get_available_liquidity(market_data, &symbol, buy_exchange, sell_exchange) {
                Ok(liq) => {
            ...
            let min_quantity = min_notional.checked_div(buy_price).unwrap_or_else(|| Decimal::ZERO);
            
            println!("Min notional: {} USD, Min quantity: {}, Available: {}", min_notional, min_quantity, liquidity);
            
            if liquidity < min_quantity {
                println!("Insufficient liquidity: {} < {}", liquidity, min_quantity);
                debug!("Insufficient liquidity for {}: {} < {}", symbol, liquidity, min_quantity);
                continue;
            }
            ...
            signal.add_metadata("estimated_fees_bps", json!(gross_profit_bps - net_profit_bps));
            
            signals.push(signal);
            
            println!("Signal created and added! Total signals: {}", signals.len());
            
            debug!("CEX arbitrage signal: {} buy@{} {} sell@{} {} liquidity={} net_profit={}bps", 
                   symbol, buy_exchange, buy_price, sell_exchange, sell_price, 
                   liquidity, net_profit_bps);
        }
    ...
    fn filter(&self, signal: &RawSignal, context: &FilterContext) -> Result<bool> {
        println!("Filtering signal: strategy_id={}, legs={}, profit_bps={}", 
                 signal.strategy_id, signal.legs.len(), signal.expected_profit_bps);
        ...
        println!("Buy leg: {} {:?} @ {}, Sell leg: {} {:?} @ {}", 
                 buy_leg.exchange, buy_leg.side, buy_leg.price,
                 sell_leg.exchange, sell_leg.side, sell_leg.price);
        
        // Validate exchanges are allowed
        if !context.are_exchanges_allowed(buy_leg.exchange, sell_leg.exchange) {
            println!("Exchanges not allowed: {} and {}", buy_leg.exchange, sell_leg.exchange);
            return Ok(false);
        }
        ...
        if signal.expected_profit_bps < context.min_profit_bps {
            println!("Profit too low: {} < {}", signal.expected_profit_bps, context.min_profit_bps);
            return Ok(false);
        }
        
        // Check inventory limits if inventory-based
        let inventory_based = self.config.custom_params.get("inventory_based")
        ...
        if inventory_based {
            ...
            println!("Checking inventory for base: {}, quote: {}", base_asset, quote_asset);
            
            // Check if we can sell base asset on sell exchange
            if !context.can_sell(sell_leg.exchange, base_asset, sell_leg.quantity) {
                println!("Cannot sell {} {} on {}", sell_leg.quantity, base_asset, sell_leg.exchange);
                return Ok(false);
            }
            ...
            if !context.can_sell(buy_leg.exchange, quote_asset, required_quote) {
                println!("Cannot buy with {} {} on {}", required_quote, quote_asset, buy_leg.exchange);
                return Ok(false);
            }
        }
        ...
        let max_latency_ms = self.get_custom_param_u64("max_latency_ms", CexArbitrageDefaults::MAX_LATENCY_MS);
        if context.max_latency_ms > max_latency_ms {
            return Ok(false);
        }
        
        // Check minimum notional
        let min_notional = self.get_custom_param_decimal("min_notional_usd", 
            Decimal::try_from(CexArbitrageDefaults::MIN_NOTIONAL_USD).unwrap_or_else(|_| Decimal::from(10)));
