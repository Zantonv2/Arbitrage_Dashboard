use crate::strategies::{Strategy, MarketBundle, RawSignal, FilterContext, StrategyConfig, TradeLeg, FundingRate};
use crate::strategies::strategies_specifics::{StrategyLimits, StrategyUtils, ExchangeCapabilities};
use crate::{ArbitrageError, Result, ExchangeId, Symbol, Side};
...
impl HedgedFundingStrategy {
    ...
    pub fn new() -> Self {
        ...
        config.min_profit_bps = StrategyLimits::get_min_profit_bps("hedged_funding");
        config.max_exposure = StrategyLimits::get_max_exposure("hedged_funding");
        
        // Add hedged funding specific parameters
        let mut custom_params = StrategyUtils::create_base_custom_params();
        custom_params.insert("min_funding_rate_pct".to_string(), json!(0.01)); // 0.01% minimum
        custom_params.insert("max_funding_rate_pct".to_string(), json!(0.5)); // 0.5% maximum
        custom_params.insert("hedge_ratio_target".to_string(), json!(1.0)); // 100% hedge target
        custom_params.insert("hedge_ratio_tolerance".to_string(), json!(0.05)); // 5% tolerance
        custom_params.insert("max_basis_risk_bps".to_string(), json!(50)); // 0.5% max basis risk
        custom_params.insert("min_time_to_funding_hours".to_string(), json!(2.0)); // 2 hours minimum
        custom_params.insert("max_position_concentration".to_string(), json!(0.4)); // 40% max per exchange
        custom_params.insert("rebalance_threshold_bps".to_string(), json!(25)); // 0.25% rebalance threshold
        custom_params.insert("funding_prediction_weight".to_string(), json!(0.3)); // 30% prediction weight
    ...
    fn update_funding_history(&mut self, market_data: &MarketBundle) {
        ...
        for ((exchange, symbol), funding_rate) in &market_data.funding_rates {
            ...
            let history = self.funding_history.entry(key).or_insert_with(Vec::new);
    ...
    fn predict_funding_rate(&self, exchange: ExchangeId, symbol: &Symbol) -> Option<Decimal> {
        ...
        let recent_rates: Vec<Decimal> = history.iter()
            .rev()
            .take(3)
            .map(|(_, rate)| *rate)
            .collect();
        ...
        let prediction_weight = self.config.custom_params.get("funding_prediction_weight")
            .and_then(|v| v.as_f64())
            .and_then(|f| Decimal::try_from(f).ok())
            .unwrap_or_else(|| Decimal::new(3, 1)); // 0.3
        
        let current_rate = recent_rates.first().copied().unwrap_or(Decimal::ZERO);
        let predicted_rate = current_rate * (Decimal::ONE - prediction_weight) + avg_rate * prediction_weight;
    ...
    fn calculate_hedge_ratio(&mut self, symbol: &Symbol, _market_data: &MarketBundle) -> Decimal {
        ...
        let target_ratio = self.config.custom_params.get("hedge_ratio_target")
            .and_then(|v| v.as_f64())
            .and_then(|f| Decimal::try_from(f).ok())
    ...
    fn calculate_basis_risk(&self, market_data: &MarketBundle, perp_exchange: ExchangeId, 
                           spot_exchange: ExchangeId, symbol: &Symbol) -> Option<Decimal> {
        // Get perpetual price (use ticker mid price as approximation)
        let perp_price = market_data.get_ticker(perp_exchange, symbol)
            .map(|t| (t.bid + t.ask) / Decimal::from(2))?;
        
        // Get spot price
        let spot_price = market_data.get_ticker(spot_exchange, symbol)
            .map(|t| (t.bid + t.ask) / Decimal::from(2))?;
    ...
    fn find_optimal_hedge_exchange(&self, market_data: &MarketBundle, perp_exchange: ExchangeId, 
                                  symbol: &Symbol) -> Option<ExchangeId> {
        ...
        for exchange in [ExchangeId::OKX, ExchangeId::ByBit, ExchangeId::MEXC, 
                        ExchangeId::GateIo, ExchangeId::Kraken, ExchangeId::Bitstamp] {
            if exchange == perp_exchange {
                continue; // Don't hedge on same exchange
            }
            
            if let Some(basis_risk) = self.calculate_basis_risk(market_data, perp_exchange, exchange, symbol) {
        ...
        let max_basis_risk_bps = self.config.custom_params.get("max_basis_risk_bps")
            .and_then(|v| v.as_i64())
            .unwrap_or(50) as i32;
    ...
    fn calculate_position_size(&self, funding_rate: &FundingRate, market_data: &MarketBundle) -> Result<Decimal> {
        // Get current price for notional calculation
        let current_price = market_data.get_ticker(funding_rate.exchange, &funding_rate.symbol)
            .map(|t| (t.bid + t.ask) / Decimal::from(2))
            .ok_or_else(|| ArbitrageError::Validation("No price data for funding rate symbol".to_string()))?;
        
        // Calculate maximum position based on concentration limits
        let max_concentration = self.config.custom_params.get("max_position_concentration")
            .and_then(|v| v.as_f64())
            .and_then(|f| Decimal::try_from(f).ok())
    ...
    fn is_funding_rate_attractive(&self, funding_rate: &FundingRate) -> bool {
        let min_rate_pct = self.config.custom_params.get("min_funding_rate_pct")
            .and_then(|v| v.as_f64())
            .unwrap_or(0.01);
            
        let max_rate_pct = self.config.custom_params.get("max_funding_rate_pct")
            .and_then(|v| v.as_f64())
            .unwrap_or(0.5);
    ...
    fn has_sufficient_time_to_funding(&self, funding_rate: &FundingRate) -> bool {
        let min_time_hours = self.config.custom_params.get("min_time_to_funding_hours")
            .and_then(|v| v.as_f64())
            .unwrap_or(2.0);
        
        let time_to_funding = (funding_rate.next_funding - Utc::now()).num_seconds() as f64 / 3600.0;
    ...
    fn find_hedged_funding_opportunities(&mut self, market_data: &MarketBundle) -> Vec<RawSignal> {
        ...
        for ((exchange, symbol), funding_rate) in &market_data.funding_rates {
            ...
            let hedge_exchange = match self.find_optimal_hedge_exchange(market_data, *exchange, symbol) {
                Some(ex) => ex,
                None => continue,
            };
            ...
            // Get prices
            let perp_price = market_data.get_ticker(*exchange, symbol)
                .map(|t| (t.bid + t.ask) / Decimal::from(2));
            let spot_price = market_data.get_ticker(hedge_exchange, symbol)
                .map(|t| (t.bid + t.ask) / Decimal::from(2));
            ...
            signal.add_metadata("time_to_funding_hours", json!((funding_rate.next_funding - Utc::now()).num_seconds() as f64 / 3600.0));
            signal.add_metadata("predicted_funding", json!(self.predict_funding_rate(*exchange, symbol).map(|r| r.to_string()).unwrap_or_else(|| "N/A".to_string())));
            signal.add_metadata("basis_risk_bps", json!(self.calculate_basis_risk(market_data, *exchange, hedge_exchange, symbol).map(|r| (r * Decimal::from(10000)).to_i32().unwrap_or(0)).unwrap_or(0)));
            
            signals.push(signal);
            
            debug!("Hedged funding opportunity: {} perp@{} hedge@{} rate={} profit={}bps", 
...
impl Strategy for HedgedFundingStrategy {
    ...
    fn detect(&self, market_data: &MarketBundle) -> Result<Vec<RawSignal>> {
        ...
        for ((exchange, symbol), funding_rate) in &market_data.funding_rates {
            ...
            for ex in [ExchangeId::OKX, ExchangeId::ByBit, ExchangeId::MEXC, 
                      ExchangeId::GateIo, ExchangeId::Kraken, ExchangeId::Bitstamp] {
    ...
    fn filter(&self, signal: &RawSignal, context: &FilterContext) -> Result<bool> {
        ...
        match perp_leg.side {
            Side::Buy => {
                let required_quote = perp_leg.price.checked_mul(perp_leg.quantity)
        ...
        match hedge_leg.side {
            Side::Buy => {
                let required_quote = hedge_leg.price.checked_mul(hedge_leg.quantity)
                    .unwrap_or(Decimal::ZERO);
