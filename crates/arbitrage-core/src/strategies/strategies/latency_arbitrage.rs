...
use crate::strategies::strategies_specifics::{StrategyLimits, ExchangeCapabilities};
...
pub struct LatencyArbitrageStrategy {
    ...
    /// Track exchange latency characteristics
    exchange_latencies: HashMap<ExchangeId, Duration>,
}
...
impl LatencyArbitrageStrategy {
    ...
    pub fn new() -> Self {
        ...
        config.min_profit_bps = StrategyLimits::get_min_profit_bps("latency_arbitrage");
        config.max_exposure = StrategyLimits::get_max_exposure("latency_arbitrage");
        
        // Add latency-specific parameters
        let custom_params = json!({
            "min_latency_spread_bps": 20,  // 0.20% minimum spread
            "max_staleness_ms": 500,       // 500ms max staleness
            "min_price_movement_bps": 5,   // 0.05% minimum movement
            "correlation_window_seconds": 10,
            "price_staleness_threshold_ms": 100
        });
        
        if let Some(obj) = custom_params.as_object() {
            config.custom_params = obj.clone().into_iter().collect();
        }
...
impl Strategy for LatencyArbitrageStrategy {
    ...
    fn detect(&self, market_data: &MarketBundle) -> Result<Vec<RawSignal>> {
        ...
        for symbol in symbols {
            ...
            let (max_exchange, max_price) = exchange_prices.iter()
                .max_by_key(|(_, price)| *price)
                .copied()
                .unwrap();
            let (min_exchange, min_price) = exchange_prices.iter()
            ...
            let min_latency_spread_bps = self.config.custom_params.get("min_latency_spread_bps")
                .and_then(|v| v.as_i64())
            ...
            if price_diff_bps >= min_latency_spread_bps {
                ...
                if let (Some(buy_book), Some(sell_book)) = (buy_book, sell_book) {
                    if let (Some(ask_level), Some(bid_level)) = (buy_book.best_ask(), sell_book.best_bid()) {
                        // Estimate fees
                        ...
                        if net_profit_bps >= self.config.min_profit_bps {
                            ...
                            if quantity >= min_quantity {
                                ...
                                signal.add_metadata("slow_exchange", json!(min_exchange.to_string()));
                                signal.add_metadata("fast_exchange", json!(max_exchange.to_string()));
