use crate::strategies::{Strategy, MarketBundle, RawSignal, FilterContext, StrategyConfig, TradeLeg, Ticker};
...
impl NewListingArbitrageStrategy {
    ...
    pub fn new() -> Self {
        ...
        // Set strategy-specific limits (higher minimum profit due to higher risk)
        config.min_profit_bps = 50; // 0.5% minimum due to volatility
        config.max_exposure = StrategyLimits::get_max_exposure("cex_arbitrage") / Decimal::from(4); // Lower exposure due to risk
        
        // Add new listing specific parameters
        let mut custom_params = StrategyUtils::create_base_custom_params();
        custom_params.insert("new_listing_window_hours".to_string(), json!(24)); // 24 hours after first seen
        custom_params.insert("min_volume_usd".to_string(), json!(10000)); // $10k minimum volume
        custom_params.insert("max_spread_bps".to_string(), json!(1000)); // 10% max spread
        custom_params.insert("volatility_threshold_pct".to_string(), json!(5.0)); // 5% volatility threshold
        custom_params.insert("early_listing_exchanges".to_string(), json!(["MEXC", "GateIo"]));
        custom_params.insert("min_price_usd".to_string(), json!(0.0001)); // Minimum price $0.0001
        custom_params.insert("max_price_usd".to_string(), json!(1000.0)); // Maximum price $1000
    ...
    fn is_newly_listed(&self, symbol: &Symbol) -> bool {
        let window_hours = self.config.custom_params.get("new_listing_window_hours")
            .and_then(|v| v.as_i64())
            .unwrap_or(24);
    ...
    fn is_valid_new_listing(&self, symbol: &Symbol, ticker: &Ticker) -> bool {
        ...
        let min_volume_usd = self.config.custom_params.get("min_volume_usd")
            .and_then(|v| v.as_f64())
        ...
        let min_price_usd = self.config.custom_params.get("min_price_usd")
            .and_then(|v| v.as_f64())
            .unwrap_or(0.00001); // Very low minimum
        let max_price_usd = self.config.custom_params.get("max_price_usd")
            .and_then(|v| v.as_f64())
            .unwrap_or(100000.0); // High maximum
        
        let mid_price = ((ticker.bid + ticker.ask) / Decimal::from(2)).to_f64().unwrap_or(0.0);
    ...
    fn find_new_listing_opportunities(&self, market_data: &MarketBundle) -> Vec<RawSignal> {
        ...
        for ((exchange, symbol), ticker) in &market_data.tickers {
            if self.is_newly_listed(symbol) && self.is_valid_new_listing(symbol, ticker) {
                symbol_tickers.entry(symbol.clone())
        ...
        for (symbol, exchange_tickers) in symbol_tickers {
            ...
            if let (Some((sell_exchange, sell_price)), Some((buy_exchange, buy_price))) = (best_bid, best_ask) {
                if sell_exchange != buy_exchange && sell_price > buy_price {
                    ...
                    if net_profit_bps >= self.config.min_profit_bps {
                        ...
                        debug!("New listing arbitrage: {} buy@{} {} sell@{} {} profit={}bps", 
                               symbol, buy_exchange, buy_price, sell_exchange, sell_price, net_profit_bps);
                    }
                }
            }
        }
...
impl Strategy for NewListingArbitrageStrategy {
    ...
    fn detect(&self, market_data: &MarketBundle) -> Result<Vec<RawSignal>> {
        ...
        let current_symbols: Vec<Symbol> = market_data.tickers.keys()
            .map(|(_, symbol)| symbol.clone())
            .collect::<HashSet<_>>()
            .into_iter()
            .collect();
        
        debug!("New listing arbitrage scanning {} symbols", current_symbols.len());
    ...
    fn filter(&self, signal: &RawSignal, context: &FilterContext) -> Result<bool> {
        ...
        let volatility_threshold = self.config.custom_params.get("volatility_threshold_pct")
            .and_then(|v| v.as_f64())
            .unwrap_or(5.0);
        ...
        // For buy leg
        let required_quote = buy_leg.price.checked_mul(buy_leg.quantity)
            .unwrap_or(Decimal::ZERO);
