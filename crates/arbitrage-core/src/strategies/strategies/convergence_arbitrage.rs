...
use crate::{Result, ExchangeId, Symbol, Side};
...
impl ConvergenceArbitrageStrategy {
    ...
    pub fn new() -> Self {
        ...
        config.min_profit_bps = StrategyLimits::get_min_profit_bps("convergence_arbitrage");
        config.max_exposure = StrategyLimits::get_max_exposure("convergence_arbitrage");
        
        // Add convergence arbitrage specific parameters
        let mut custom_params = StrategyUtils::create_base_custom_params();
        custom_params.insert("min_correlation".to_string(), json!(0.7)); // 70% minimum correlation
        custom_params.insert("z_score_entry_threshold".to_string(), json!(2.0)); // 2 standard deviations
        custom_params.insert("z_score_exit_threshold".to_string(), json!(0.5)); // 0.5 standard deviations
        custom_params.insert("lookback_period_hours".to_string(), json!(24)); // 24 hour lookback
        custom_params.insert("min_data_points".to_string(), json!(100)); // Minimum data points for analysis
        custom_params.insert("max_holding_period_hours".to_string(), json!(48)); // Maximum 48 hour hold
        custom_params.insert("convergence_confidence_threshold".to_string(), json!(0.8)); // 80% confidence
        custom_params.insert("pair_volatility_threshold".to_string(), json!(0.05)); // 5% max volatility
    ...
    fn update_price_history(&mut self, market_data: &MarketBundle) {
        ...
        for ((exchange, symbol), ticker) in &market_data.tickers {
            ...
            let lookback_hours = self.config.custom_params.get("lookback_period_hours")
                .and_then(|v| v.as_i64())
                .unwrap_or(24);
    ...
    fn calculate_correlation(&self, symbol1: &Symbol, symbol2: &Symbol) -> Option<Decimal> {
        ...
        let sum1_sq: f64 = returns1.iter().map(|r| r.to_f64().unwrap_or(0.0).powi(2)).sum();
        let sum2_sq: f64 = returns2.iter().map(|r| r.to_f64().unwrap_or(0.0).powi(2)).sum();
        let sum_prod: f64 = returns1.iter().zip(&returns2)
    ...
    fn calculate_z_score(&self, symbol1: &Symbol, symbol2: &Symbol, current_ratio: Decimal) -> Option<Decimal> {
        let prices1 = self.get_price_series(symbol1)?;
        let prices2 = self.get_price_series(symbol2)?;
        ...
        let variance: Decimal = ratios.iter()
            .map(|r| {
                let diff = *r - mean;
                diff * diff // Use multiplication instead of powi
            })
            .sum::<Decimal>() / Decimal::from(ratios.len());
    ...
    fn find_convergence_opportunities(&mut self, market_data: &MarketBundle) -> Vec<RawSignal> {
        ...
        for i in 0..symbols.len() {
            for j in (i+1)..symbols.len() {
                ...
                let min_correlation = self.config.custom_params.get("min_correlation")
                    .and_then(|v| v.as_f64())
                ...
                let z_threshold = self.config.custom_params.get("z_score_entry_threshold")
                    .and_then(|v| v.as_f64())
                    .and_then(|f| Decimal::try_from(f).ok())
                ...
                let (long_symbol, short_symbol, long_price, short_price) = if z_score > Decimal::ZERO {
                ...
                let common_exchanges = self.find_common_exchanges(market_data, long_symbol, short_symbol);
                ...
                for exchange in common_exchanges {
                    ...
                    let expected_convergence_bps = (z_score.abs() * Decimal::from(50))
                        .to_i32().unwrap_or(0).min(500); // Cap at 5%
                    ...
                    debug!("Convergence arbitrage: {} vs {} correlation={} z_score={} profit={}bps", 
    ...
    fn find_common_exchanges(&self, market_data: &MarketBundle, symbol1: &Symbol, symbol2: &Symbol) -> Vec<ExchangeId> {
        ...
        for ((exchange, sym), _) in &market_data.tickers {
            if sym == symbol1 {
...
impl Strategy for ConvergenceArbitrageStrategy {
    ...
    fn detect(&self, market_data: &MarketBundle) -> Result<Vec<RawSignal>> {
        ...
        for i in 0..symbols.len() {
            for j in (i+1)..symbols.len() {
                ...
                let (long_symbol, short_symbol, long_price, short_price) = if current_ratio > Decimal::ONE {
                ...
                for exchange in common_exchanges {
                    ...
                    debug!("Convergence arbitrage: {} vs {} ratio={} deviation={} profit={}bps", 
                           long_symbol, short_symbol, current_ratio, ratio_deviation, expected_convergence_bps);
                }
            }
        }
    ...
    fn filter(&self, signal: &RawSignal, context: &FilterContext) -> Result<bool> {
        ...
        let required_quote = long_leg.price.checked_mul(long_leg.quantity)
