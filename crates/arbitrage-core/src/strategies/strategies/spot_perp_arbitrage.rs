...
use crate::strategies::strategies_specifics::{ExchangeCapabilities, StrategyLimits, StrategyUtils};
...
impl SpotPerpArbitrageStrategy {
    ...
    pub fn new() -> Self {
        ...
        // Set strategy-specific limits - more permissive for testing
        config.min_profit_bps = 5; // 0.05% minimum profit (lower than default)
        config.max_exposure = StrategyLimits::get_max_exposure("spot_perp_arbitrage");
        
        // Add spot-perp specific parameters - more permissive thresholds
        let mut custom_params = StrategyUtils::create_base_custom_params();
        custom_params.insert("min_basis_bps".to_string(), json!(5)); // 0.05% minimum basis (lower)
        custom_params.insert("max_basis_bps".to_string(), json!(1000)); // 10.00% maximum basis (higher)
        custom_params.insert("funding_weight".to_string(), json!(0.3)); // 30% weight for funding
        custom_params.insert("convergence_days".to_string(), json!(7)); // Expected convergence in 7 days
        custom_params.insert("margin_buffer_pct".to_string(), json!(0.2)); // 20% margin buffer
    ...
    fn calculate_basis_bps(&self, perp_price: Decimal, spot_price: Decimal) -> Result<i32> {
        if spot_price.is_zero() {
            return Err(ArbitrageError::Calculation("Spot price cannot be zero".to_string()));
        }
        
        let basis = perp_price - spot_price;
        let basis_ratio = basis.checked_div(spot_price)
            .ok_or_else(|| ArbitrageError::Calculation("Division by zero in basis calculation".to_string()))?;
        
        let basis_bps = basis_ratio.checked_mul(Decimal::from(10000))
            .ok_or_else(|| ArbitrageError::Calculation("Basis calculation overflow".to_string()))?
            .to_i32()
            .ok_or_else(|| ArbitrageError::Calculation("Basis BPS conversion failed".to_string()))?;
    ...
    fn is_basis_valid(&self, basis_bps: i32) -> bool {
        let min_basis_bps = self.config.custom_params.get("min_basis_bps")
            .and_then(|v| v.as_i64())
            .unwrap_or(20) as i32;
            
        let max_basis_bps = self.config.custom_params.get("max_basis_bps")
            .and_then(|v| v.as_i64())
    ...
    fn get_spot_price(&self, market_data: &MarketBundle, symbol: &Symbol, exchange: ExchangeId) -> Option<Decimal> {
        // Prioritize spot exchanges for spot prices
        let spot_exchanges = vec![ExchangeId::Bitstamp, ExchangeId::Kraken];
        let perp_exchanges = vec![ExchangeId::ByBit, ExchangeId::OKX, ExchangeId::MEXC];
    ...
    fn get_perp_price(&self, market_data: &MarketBundle, base_symbol: &Symbol, exchange: ExchangeId) -> Option<Decimal> {
        // First try to find actual perpetual symbols
        let perp_symbols = vec![
        ...
        if let Some(spot_price) = self.get_spot_price(market_data, base_symbol, exchange) {
            ...
            if let Some(funding_rate) = market_data.get_funding_rate(exchange, base_symbol) {
                ...
                let funding_bps = (funding_rate.rate * Decimal::from(10000)).to_i32().unwrap_or(0);
    ...
    fn calculate_expected_profit_bps(&self, basis_bps: i32, funding_rate: Option<&FundingRate>) -> Result<i32> {
        let mut expected_profit = basis_bps;
        
        // Adjust for funding rate if available
        if let Some(funding) = funding_rate {
            let funding_weight = self.config.custom_params.get("funding_weight")
                .and_then(|v| v.as_f64())
                .unwrap_or(0.3);
            
            let convergence_days = self.config.custom_params.get("convergence_days")
                .and_then(|v| v.as_f64())
                .unwrap_or(7.0);
            ...
            let total_funding_bps = (funding.rate.to_f64().unwrap_or(0.0) * funding_periods * 10000.0) as i32;
    ...
    fn get_available_liquidity(&self, market_data: &MarketBundle, spot_symbol: &Symbol, 
                              exchange: ExchangeId, side: Side) -> Result<Decimal> {
        let spot_book = market_data.get_order_book(exchange, spot_symbol)
            .ok_or_else(|| ArbitrageError::Validation("Spot order book not found".to_string()))?;
        
        let spot_liquidity = match side {
            Side::Buy => spot_book.best_ask().map(|l| l.quantity).unwrap_or(Decimal::ZERO),
            Side::Sell => spot_book.best_bid().map(|l| l.quantity).unwrap_or(Decimal::ZERO),
        };
    ...
    fn calculate_position_size(&self, available_liquidity: Decimal, price: Decimal) -> Decimal {
        let margin_buffer_pct = self.config.custom_params.get("margin_buffer_pct")
            .and_then(|v| v.as_f64())
            .unwrap_or(0.2);
        ...
        // Apply margin buffer
        let buffer_multiplier = Decimal::ONE - Decimal::try_from(margin_buffer_pct).unwrap_or(Decimal::ZERO);
        let adjusted_liquidity = available_liquidity.checked_mul(buffer_multiplier).unwrap_or(available_liquidity);
...
impl Strategy for SpotPerpArbitrageStrategy {
    ...
    fn detect(&self, market_data: &MarketBundle) -> Result<Vec<RawSignal>> {
        ...
        debug!("Spot-perp arbitrage found {} unique symbols", all_symbols.len());
        ...
        for symbol in all_symbols {
            ...
            for exchange in [ExchangeId::Bitstamp, ExchangeId::ByBit, ExchangeId::OKX, ExchangeId::MEXC, ExchangeId::Kraken, ExchangeId::GateIo] {
                ...
                if let Some(ticker) = market_data.get_ticker(exchange, &symbol) {
                    if ticker.bid > Decimal::ZERO && ticker.ask > Decimal::ZERO {
                        ...
                        if !exchange_data.iter().any(|(ex, _)| *ex == exchange) {
                            exchange_data.push((exchange, mid_price));
                            debug!("Found {} on {} at ${} (from ticker)", symbol, exchange, mid_price);
                        }
                    }
                }
            }
            
            debug!("Symbol {} found on {} exchanges", symbol, exchange_data.len());
            ...
            for i in 0..exchange_data.len() {
                for j in (i + 1)..exchange_data.len() {
                    ...
                    debug!("Checking {} spot@{} ${} vs perp@{} ${}, basis={}bps", 
                    ...
                    if let Some(funding) = funding_rate {
                        let funding_bps = (funding.rate * Decimal::from(10000)).to_i32().unwrap_or(0);
                        // Simple adjustment: if funding is positive and we're long perp, subtract some profit
                        if basis_bps > 0 && funding_bps > 0 {
                            expected_profit_bps = (expected_profit_bps - funding_bps.abs() / 4).max(0);
                        }
                    }
                    
                    debug!("Expected profit: {}bps (min required: {}bps)", expected_profit_bps, self.config.min_profit_bps);
                    
                    // Check minimum profit threshold
                    if expected_profit_bps < self.config.min_profit_bps {
                        debug!("Profit too low: {}bps < {}bps", expected_profit_bps, self.config.min_profit_bps);
                        continue;
                    }
    ...
    fn filter(&self, signal: &RawSignal, context: &FilterContext) -> Result<bool> {
        ...
        if !context.is_exchange_allowed(leg1.exchange) || !context.is_exchange_allowed(leg2.exchange) {
            return Ok(false);
        }
        ...
        match leg1.side {
            Side::Buy => {
                let required_quote = leg1.price.checked_mul(leg1.quantity).unwrap_or(Decimal::ZERO);
        ...
        match leg2.side {
            Side::Buy => {
                let required_quote = leg2.price.checked_mul(leg2.quantity).unwrap_or(Decimal::ZERO);
