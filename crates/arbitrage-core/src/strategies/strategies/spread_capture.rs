...
use crate::{ArbitrageError, Result, ExchangeId, Symbol, Side, OrderType};
...
impl SpreadCaptureStrategy {
    ...
    pub fn new() -> Self {
        ...
        config.min_profit_bps = StrategyLimits::get_min_profit_bps("spread_capture");
        config.max_exposure = StrategyLimits::get_max_exposure("spread_capture");
        ...
        custom_params.insert("maker_fee_threshold_bps".to_string(), json!(-5)); // Require maker rebate
    ...
    fn calculate_spread_bps(&self, bid: Decimal, ask: Decimal) -> Result<i32> {
        if bid.is_zero() || ask.is_zero() || ask <= bid {
            return Err(ArbitrageError::Calculation("Invalid bid/ask prices".to_string()));
        }
        
        let mid_price = (bid + ask) / Decimal::from(2);
        let spread = ask - bid;
        
        let spread_ratio = spread.checked_div(mid_price)
        ...
        let spread_bps = (spread_ratio * Decimal::from(10000))
            .to_i32()
            .ok_or_else(|| ArbitrageError::Calculation("Spread BPS conversion failed".to_string()))?;
    ...
    fn is_spread_valid(&self, spread_bps: i32) -> bool {
        let min_spread_bps = self.config.custom_params.get("min_spread_bps")
            .and_then(|v| v.as_i64())
            .unwrap_or(20) as i32;
            
        let max_spread_bps = self.config.custom_params.get("max_spread_bps")
            .and_then(|v| v.as_i64())
    ...
    fn calculate_order_book_depth(&self, market_data: &MarketBundle, exchange: ExchangeId, symbol: &Symbol) -> Result<Decimal> {
    ...
    fn has_sufficient_depth(&self, depth_usd: Decimal) -> bool {
        let min_depth = self.config.custom_params.get("min_order_book_depth")
            .and_then(|v| v.as_f64())
    ...
    fn calculate_maker_prices(&self, bid: Decimal, ask: Decimal) -> Result<(Decimal, Decimal)> {
        let spread_capture_ratio = self.config.custom_params.get("spread_capture_ratio")
            .and_then(|v| v.as_f64())
            .and_then(|f| Decimal::try_from(f).ok())
            .unwrap_or_else(|| Decimal::new(6, 1)); // 0.6
        
        let spread = ask - bid;
        let capture_amount = spread.checked_mul(spread_capture_ratio)
            .ok_or_else(|| ArbitrageError::Calculation("Spread capture calculation overflow".to_string()))?;
        ...
        if our_bid >= our_ask {
            return Err(ArbitrageError::Calculation("Invalid maker prices: bid >= ask".to_string()));
        }
    ...
    fn calculate_position_size(&self, price: Decimal) -> Result<Decimal> {
        let position_size_usd = self.config.custom_params.get("position_size_usd")
            .and_then(|v| v.as_f64())
            .and_then(|f| Decimal::try_from(f).ok())
            .unwrap_or_else(|| Decimal::from(500));
        
        let quantity = position_size_usd.checked_div(price)
            .ok_or_else(|| ArbitrageError::Calculation("Position size calculation failed".to_string()))?;
        
        Ok(quantity)
    }
    
    /// Estimate profit from spread capture
    fn estimate_spread_profit_bps(&self, our_bid: Decimal, our_ask: Decimal, exchange: ExchangeId) -> Result<i32> {
        ...
        let gross_profit_ratio = gross_profit.checked_div(mid_price)
            .ok_or_else(|| ArbitrageError::Calculation("Profit ratio calculation failed".to_string()))?;
    ...
    fn has_favorable_maker_fees(&self, exchange: ExchangeId) -> bool {
        let maker_fee_bps = self.get_maker_fee_bps(exchange);
        let threshold_bps = self.config.custom_params.get("maker_fee_threshold_bps")
            .and_then(|v| v.as_i64())
            .unwrap_or(-5) as i32;
    ...
    fn find_spread_opportunities(&self, market_data: &MarketBundle) -> Vec<RawSignal> {
        ...
        for symbol in symbols {
            ...
            for exchange in [ExchangeId::OKX, ExchangeId::ByBit, ExchangeId::MEXC, ExchangeId::GateIo] {
                ...
                let depth_usd = match self.calculate_order_book_depth(market_data, exchange, &symbol) {
                    Ok(depth) => depth,
                    Err(_) => continue,
                };
                ...
                debug!("Spread capture opportunity: {} on {} spread={}bps profit={}bps", 
...
impl Strategy for SpreadCaptureStrategy {
    ...
    fn detect(&self, market_data: &MarketBundle) -> Result<Vec<RawSignal>> {
        debug!("Spread capture scanning {} order books", market_data.order_books.len());
    ...
    fn filter(&self, signal: &RawSignal, context: &FilterContext) -> Result<bool> {
        ...
        let required_quote = bid_leg.price.checked_mul(bid_leg.quantity)
