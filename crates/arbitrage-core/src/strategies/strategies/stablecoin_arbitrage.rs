...
use crate::strategies::strategies_specifics::{StablecoinDefaults, ExchangeCapabilities, StrategyLimits};
...
impl StablecoinArbitrageStrategy {
    ...
    pub fn new() -> Self {
        ...
        config.min_profit_bps = StrategyLimits::get_min_profit_bps("stablecoin_arbitrage");
        config.max_exposure = StrategyLimits::get_max_exposure("stablecoin_arbitrage");
        
        // Add stablecoin specific parameters
        config.custom_params = StablecoinDefaults::get_custom_params();
    ...
    fn calculate_peg_deviation_bps(&self, symbol: &Symbol, price: Decimal) -> Result<i32> {
        ...
        let target_price = peg_targets.get(&symbol.base)
            .copied()
            .unwrap_or(Decimal::ONE);
        
        if target_price.is_zero() {
            return Err(ArbitrageError::Calculation("Target price cannot be zero".to_string()));
        }
        
        let deviation = (price - target_price).checked_div(target_price)
            .ok_or_else(|| ArbitrageError::Calculation("Division by zero in peg deviation".to_string()))?;
        
        let deviation_bps = deviation.checked_mul(Decimal::from(10000))
            .ok_or_else(|| ArbitrageError::Calculation("Peg deviation overflow".to_string()))?
            .to_i32()
            .ok_or_else(|| ArbitrageError::Calculation("Peg deviation conversion failed".to_string()))?;
    ...
    fn is_deviation_valid(&self, deviation_bps: i32) -> bool {
        let min_deviation_pct = self.config.custom_params.get("min_peg_deviation_pct")
            .and_then(|v| v.as_f64())
            .unwrap_or(StablecoinDefaults::MIN_PEG_DEVIATION_PCT);
            
        let max_deviation_pct = self.config.custom_params.get("max_peg_deviation_pct")
            .and_then(|v| v.as_f64())
    ...
    fn find_peg_arbitrage(&self, market_data: &MarketBundle, symbol: &Symbol, exchange: ExchangeId) -> Option<RawSignal> {
    ...
    fn find_cross_stablecoin_arbitrage(&self, market_data: &MarketBundle) -> Vec<RawSignal> {
        ...
        for base in &stablecoins {
            for quote in &stablecoins {
                ...
                for exchange in self.get_supported_exchanges() {
                    if let Some(order_book) = market_data.get_order_book(exchange, &symbol) {
                        if let Some(bid_level) = order_book.best_bid() {
                            match best_bid {
                                None => best_bid = Some((exchange, bid_level.price, bid_level.quantity)),
                        ...
                        if let Some(ask_level) = order_book.best_ask() {
                            match best_ask {
                                None => best_ask = Some((exchange, ask_level.price, ask_level.quantity)),
                                Some((_, current_price, _)) => {
                ...
                if let (Some((sell_exchange, sell_price, sell_qty)), Some((buy_exchange, buy_price, buy_qty))) = (best_bid, best_ask) {
                    if sell_exchange != buy_exchange && sell_price > buy_price {
                        ...
                        if net_profit_bps >= self.config.min_profit_bps {
                            ...
                            // Buy leg
                            let buy_leg = TradeLeg::new(buy_exchange, symbol.clone(), Side::Buy, buy_price, quantity);
                            signal.add_leg(buy_leg);
                            
                            // Sell leg
                            let sell_leg = TradeLeg::new(sell_exchange, symbol.clone(), Side::Sell, sell_price, quantity);
...
impl Strategy for StablecoinArbitrageStrategy {
    ...
    fn detect(&self, market_data: &MarketBundle) -> Result<Vec<RawSignal>> {
        ...
        for ((exchange, symbol), _ticker) in &market_data.tickers {
        ...
        for leg in &signal.legs {
            match leg.side {
                ...
                Side::Buy => {
                    ...
                    let required_quote = leg.price.checked_mul(leg.quantity)
                        .unwrap_or(Decimal::ZERO);
