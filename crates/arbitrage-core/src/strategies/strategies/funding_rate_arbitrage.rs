...
use crate::strategies::strategies_specifics::{FundingRateDefaults, ExchangeCapabilities, StrategyLimits};
...
impl FundingRateArbitrageStrategy {
    ...
    pub fn new() -> Self {
        ...
        config.min_profit_bps = StrategyLimits::get_min_profit_bps("funding_rate_arbitrage");
        config.max_exposure = StrategyLimits::get_max_exposure("funding_rate_arbitrage");
        
        // Add funding rate specific parameters
        config.custom_params = FundingRateDefaults::get_custom_params();
    ...
    fn calculate_annualized_return(&self, funding_rate: &FundingRate) -> Result<Decimal> {
        ...
        let daily_rate = funding_rate.rate.checked_mul(Decimal::from(3))
            .ok_or_else(|| ArbitrageError::Calculation("Daily funding rate overflow".to_string()))?;
    ...
    fn time_to_funding_hours(&self, funding_rate: &FundingRate) -> Result<Decimal> {
        ...
        if duration < Duration::zero() {
            return Err(ArbitrageError::Validation("Funding time is in the past".to_string()));
        }
    ...
    fn calculate_funding_profit_bps(&self, funding_rate: &FundingRate, _time_to_funding: Decimal) -> Result<i32> {
        ...
        let funding_bps = funding_rate.rate.checked_mul(Decimal::from(10000))
            .ok_or_else(|| ArbitrageError::Calculation("Funding rate BPS conversion overflow".to_string()))?
            .to_i32()
            .ok_or_else(|| ArbitrageError::Calculation("Funding BPS conversion failed".to_string()))?;
    ...
    fn is_funding_rate_valid(&self, funding_rate: &FundingRate) -> bool {
        ...
        let rate_pct = funding_rate.rate.checked_mul(Decimal::from(100))
    ...
    fn get_max_position_size(&self) -> Decimal {
        let max_position_usd = self.config.custom_params.get("max_funding_position_usd")
            .and_then(|v| v.as_f64())
...
impl Strategy for FundingRateArbitrageStrategy {
    ...
    fn detect(&self, market_data: &MarketBundle) -> Result<Vec<RawSignal>> {
        ...
        debug!("Funding rate arbitrage scanning {} funding rates", market_data.funding_rates.len());
        ...
        for ((exchange, symbol), funding_rate) in &market_data.funding_rates {
            ...
            if !self.is_funding_rate_valid(funding_rate) {
                debug!("Funding rate invalid for {} on {}: {}", symbol, exchange, funding_rate.rate);
                continue;
            }
            ...
            let time_to_funding = match self.time_to_funding_hours(funding_rate) {
                Ok(time) => time,
                Err(e) => {
                    warn!("Failed to calculate funding time for {} on {}: {}", symbol, exchange, e);
                    continue;
                }
            };
            ...
            if !self.has_sufficient_time(time_to_funding) {
                debug!("Insufficient time to funding for {} on {}: {} hours", symbol, exchange, time_to_funding);
                continue;
            }
            ...
            let profit_bps = match self.calculate_funding_profit_bps(funding_rate, time_to_funding) {
                Ok(profit) => profit,
                Err(e) => {
                    warn!("Failed to calculate funding profit for {} on {}: {}", symbol, exchange, e);
                    continue;
                }
            };
            ...
            if profit_bps.abs() < self.config.min_profit_bps {
                debug!("Funding profit too low for {} on {}: {}bps < {}bps", 
            ...
            let position_quantity = max_position_usd.checked_div(spot_price)
                .unwrap_or_else(|| Decimal::ZERO);
            ...
            let hedge_quantity = position_quantity.checked_mul(hedge_ratio)
                .unwrap_or_else(|| position_quantity);
            ...
            signal.add_metadata("annualized_return", json!(self.calculate_annualized_return(funding_rate)?.to_string()));
            signal.add_metadata("next_funding", json!(funding_rate.next_funding.to_rfc3339()));
            signal.add_metadata("position_side", json!(perp_side.to_string()));
            signal.add_metadata("hedge_ratio", json!(hedge_ratio.to_string()));
            
            signals.push(signal);
            
            debug!("Funding rate signal: {} {} rate={} profit={}bps time={}h", 
    ...
    fn filter(&self, signal: &RawSignal, context: &FilterContext) -> Result<bool> {
        ...
        println!("Perp leg: {} {:?} @ {}, Spot leg: {} {:?} @ {}", 
                 perp_leg.exchange, perp_leg.side, perp_leg.price,
                 spot_leg.exchange, spot_leg.side, spot_leg.price);
        ...
        if total_notional > context.max_exposure {
            println!("Exposure too high: {} > {}", total_notional, context.max_exposure);
            return Ok(false);
        }
        ...
        match spot_leg.side {
            Side::Sell => {
                ...
                println!("Spot leg sell: need {} {}, have {}", spot_leg.quantity, base_asset, available);
                if !context.can_sell(spot_leg.exchange, base_asset, spot_leg.quantity) {
                    println!("Cannot sell {} {} on {}", spot_leg.quantity, base_asset, spot_leg.exchange);
                    return Ok(false);
                }
            },
            Side::Buy => {
                let required_quote = spot_leg.price.checked_mul(spot_leg.quantity)
                    .unwrap_or_else(|| Decimal::ZERO);
                let available = context.get_inventory(spot_leg.exchange, quote_asset);
                println!("Spot leg buy: need {} {}, have {}", required_quote, quote_asset, available);
                if !context.can_sell(spot_leg.exchange, quote_asset, required_quote) {
                    println!("Cannot buy with {} {} on {}", required_quote, quote_asset, spot_leg.exchange);
                    return Ok(false);
                }
            }
        }
        ...
        if total_notional < context.min_notional_usd {
            println!("Notional too low: {} < {}", total_notional, context.min_notional_usd);
            return Ok(false);
        }
