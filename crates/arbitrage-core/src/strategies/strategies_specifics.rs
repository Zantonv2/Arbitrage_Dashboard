...
impl CexArbitrageDefaults {
    ...
    pub fn get_custom_params() -> HashMap<String, Value> {
        ...
        params.insert("min_notional_usd".to_string(), json!(Self::MIN_NOTIONAL_USD));
        params.insert("max_position_pct".to_string(), json!(Self::MAX_POSITION_PCT));
        params.insert("vwap_quantity_usd".to_string(), json!(Self::VWAP_QUANTITY_USD));
...
impl FundingRateDefaults {
    ...
    pub fn get_custom_params() -> HashMap<String, Value> {
        let mut params = HashMap::new();
        params.insert("min_funding_rate_pct".to_string(), json!(Self::MIN_FUNDING_RATE_PCT));
        params.insert("max_funding_rate_pct".to_string(), json!(Self::MAX_FUNDING_RATE_PCT));
        params.insert("min_time_to_funding_hours".to_string(), json!(Self::MIN_TIME_TO_FUNDING_HOURS));
        params.insert("max_funding_position_usd".to_string(), json!(Self::MAX_FUNDING_POSITION_USD));
        params
    }
}
...
impl StablecoinDefaults {
    ...
    pub fn get_custom_params() -> HashMap<String, Value> {
        let mut params = HashMap::new();
        params.insert("min_peg_deviation_pct".to_string(), json!(Self::MIN_PEG_DEVIATION_PCT));
        params.insert("max_peg_deviation_pct".to_string(), json!(Self::MAX_PEG_DEVIATION_PCT));
        
        let peg_targets: HashMap<String, f64> = Self::PEG_TARGETS.iter()
...
impl ExchangeCapabilities {
    ...
    pub fn supports_perpetuals(exchange: ExchangeId) -> bool {
        matches!(exchange, ExchangeId::OKX | ExchangeId::ByBit | ExchangeId::MEXC)
    }
...
impl StrategyUtils {
    ...
    pub fn merge_custom_params(base: HashMap<String, Value>, overrides: HashMap<String, Value>) -> HashMap<String, Value> {
        let mut merged = base;
