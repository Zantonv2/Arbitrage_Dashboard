use arbitrage_core::{
    ...
    strategies::{StrategyRegistry, Strategy, strategies::FundingRateArbitrageStrategy, MarketBundle, FundingRate},
...
async fn test_funding_rate_arbitrage_integration() -> Result<()> {
    ...
    assert!(!signals.is_empty(), "Should detect funding rate opportunity");
