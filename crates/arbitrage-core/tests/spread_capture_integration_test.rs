use arbitrage_core::{
    strategies::{Strategy, MarketBundle, FilterContext, strategies::SpreadCaptureStrategy, StrategyConfig, Ticker},
...
async fn test_spread_capture_integration() -> Result<()> {
    ...
    config.custom_params.insert("maker_fee_threshold_bps".to_string(), serde_json::json!(0)); // Allow up to 0 bps
    config.custom_params.insert("min_spread_bps".to_string(), serde_json::json!(20)); // 0.20% minimum spread
    config.custom_params.insert("min_order_book_depth".to_string(), serde_json::json!(1000)); // $1000 minimum depth
    ...
    assert!(!signals.is_empty(), "Should detect spread capture opportunity");
    
    let signal = &signals[0];
    assert_eq!(signal.legs.len(), 2, "Should have 2 legs (bid + ask orders)");
...
async fn test_spread_capture_filtering() -> Result<()> {
    ...
    assert!(signals.is_empty() || signals[0].expected_profit_bps < 20, 
...
async fn test_spread_capture_insufficient_depth() -> Result<()> {
    ...
    let okx_book = OrderBook::new(
        ExchangeId::OKX,
        symbol.clone(),
        vec![OrderBookLevel::new(Decimal::from(49000), Decimal::new(1, 3))], // 0.001 BTC
        vec![OrderBookLevel::new(Decimal::from(51000), Decimal::new(1, 3))], // 0.001 BTC
    );
    ...
    assert!(signals.is_empty(), "Should not detect opportunity with insufficient depth");
...
async fn test_spread_capture_multiple_exchanges() -> Result<()> {
    ...
    println!("Detected {} signals across multiple exchanges", signals.len());
    
    // Should detect opportunity on OKX but not ByBit
    let okx_signals: Vec<_> = signals.iter()
        .filter(|s| s.legs.iter().all(|leg| leg.exchange == ExchangeId::OKX))
        .collect();
    
    let bybit_signals: Vec<_> = signals.iter()
        .filter(|s| s.legs.iter().all(|leg| leg.exchange == ExchangeId::ByBit))
        .collect();
    
    assert!(!okx_signals.is_empty(), "Should detect opportunity on OKX");
    assert!(bybit_signals.is_empty(), "Should not detect opportunity on ByBit due to narrow spread");
