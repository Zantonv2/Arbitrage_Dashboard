use arbitrage_core::{
    strategies::{Strategy, MarketBundle, FilterContext, strategies::ConvergenceArbitrageStrategy, Ticker},
...
async fn test_convergence_arbitrage_integration() -> Result<()> {
    ...
    for i in 0..symbols.len() {
        for j in (i+1)..symbols.len() {
            ...
            println!("Checking pair: {} vs {}", symbol1.to_pair(), symbol2.to_pair());
            println!("  Same base: {} vs {} = {}", symbol1.base, symbol2.base, symbol1.base == symbol2.base);
            println!("  Same quote: {} vs {} = {}", symbol1.quote, symbol2.quote, symbol1.quote == symbol2.quote);
    ...
    assert!(!signals.is_empty(), "Should detect convergence arbitrage opportunity");
...
async fn test_convergence_arbitrage_no_opportunity() -> Result<()> {
    ...
    assert!(signals.is_empty(), "Should not detect opportunity with normal price ratios");
...
async fn test_convergence_arbitrage_different_quotes() -> Result<()> {
    ...
    assert!(signals.is_empty(), "Should not detect opportunity with different quote currencies");
...
async fn test_convergence_arbitrage_same_assets() -> Result<()> {
    ...
    assert!(signals.is_empty(), "Should not detect opportunity with same base assets");
...
async fn test_convergence_arbitrage_multiple_exchanges() -> Result<()> {
    ...
    assert!(!bybit_signals.is_empty(), "Should detect opportunity on ByBit");
