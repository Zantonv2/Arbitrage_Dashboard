...
async fn test_new_listing_arbitrage_established_token() -> Result<()> {
    ...
    assert!(
        signals.is_empty() || signals[0].expected_profit_bps < 100,
...
async fn test_new_listing_arbitrage_high_volatility() -> Result<()> {
    ...
    if !signals.is_empty() {
        ...
        assert!(
            signal.expected_profit_bps > 5000,
            "Should detect large profit opportunity"
        );
        ...
        let price_diff_pct = ((sell_leg.price - buy_leg.price) / buy_leg.price
            * Decimal::from(100))
        .to_f64()
        .unwrap_or(0.0);
