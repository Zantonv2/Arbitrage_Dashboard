...
async fn test_multi_strategy_detection() -> Result<()> {
    ...
    let usdt_book = OrderBook::new(
        ExchangeId::Kraken,
        usdt_symbol.clone(),
        vec![OrderBookLevel::new(Decimal::new(1003, 3), Decimal::from(5000))],
        vec![OrderBookLevel::new(Decimal::new(1005, 3), Decimal::from(5000))],
    );
    ...
    let usdt_usdc_okx = OrderBook::new(
        ...
        vec![OrderBookLevel::new(Decimal::new(1002, 3), Decimal::from(2000))],
        vec![OrderBookLevel::new(Decimal::new(1003, 3), Decimal::from(2000))],
    );
    let usdt_usdc_bybit = OrderBook::new(
        ExchangeId::ByBit,
        usdt_usdc_symbol.clone(),
        vec![OrderBookLevel::new(Decimal::new(999, 3), Decimal::from(2000))],
        vec![OrderBookLevel::new(Decimal::new(1000, 3), Decimal::from(2000))],
    );
    ...
    for strategy in strategies {
        ...
        println!("Strategy '{}' detected {} signals", strategy.name(), signals.len());
        
        for (i, signal) in signals.iter().enumerate() {
            println!("  Signal {}: {} legs, {}bps profit", 
                     i + 1, signal.legs.len(), signal.expected_profit_bps);
        }
    ...
    assert!(total_signals > 0, "Should detect arbitrage opportunities across strategies");
...
async fn test_strategy_performance() -> Result<()> {
    ...
    for i in 0..100 {
        ...
        for exchange in [ExchangeId::OKX, ExchangeId::ByBit, ExchangeId::MEXC, 
            ...
            let order_book = OrderBook::new(
                exchange,
                symbol.clone(),
                vec![OrderBookLevel::new(Decimal::from(base_price), Decimal::from(10))],
                vec![OrderBookLevel::new(Decimal::from(base_price + spread), Decimal::from(10))],
            );
    ...
    println!("Created market bundle with {} order books", market_bundle.order_books.len());
    ...
    println!("Strategy detection took {:?} for {} signals", duration, signals.len());
    
    // Should complete within reasonable time (< 100ms for this test)
    assert!(duration.as_millis() < 1000, "Strategy detection should be fast");
