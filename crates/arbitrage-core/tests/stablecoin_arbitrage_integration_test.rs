use arbitrage_core::{
    strategies::{Strategy, strategies::StablecoinArbitrageStrategy, MarketBundle, Ticker},
    types::{Symbol, ExchangeId, OrderBook, OrderBookLevel},
    Result,
};
...
async fn test_stablecoin_peg_arbitrage() -> Result<()> {
    ...
    let order_book = OrderBook::new(
        ...
        symbol.clone(),
        vec![OrderBookLevel::new(Decimal::new(10045, 4), Decimal::from(10000))], // bids
        vec![OrderBookLevel::new(Decimal::new(10055, 4), Decimal::from(10000))], // asks
    );
...
async fn test_stablecoin_below_peg() -> Result<()> {
    ...
    let order_book = OrderBook::new(
        ExchangeId::Bitstamp,
        symbol.clone(),
        vec![OrderBookLevel::new(Decimal::new(9975, 4), Decimal::from(5000))],
        vec![OrderBookLevel::new(Decimal::new(9985, 4), Decimal::from(5000))],
    );
...
async fn test_cross_stablecoin_arbitrage() -> Result<()> {
    ...
    let okx_book = OrderBook::new(
        ExchangeId::OKX,
        symbol.clone(),
        vec![OrderBookLevel::new(Decimal::new(1002, 3), Decimal::from(1000))], // bid: 1.002
        vec![OrderBookLevel::new(Decimal::new(1003, 3), Decimal::from(1000))], // ask: 1.003
    );
    ...
    let bybit_book = OrderBook::new(
        ExchangeId::ByBit,
        symbol.clone(),
        vec![OrderBookLevel::new(Decimal::new(999, 3), Decimal::from(1000))], // bid: 0.999
        vec![OrderBookLevel::new(Decimal::new(1000, 3), Decimal::from(1000))], // ask: 1.000
    );
...
async fn test_stablecoin_small_deviation() -> Result<()> {
    ...
    let order_book = OrderBook::new(
        ExchangeId::Kraken,
        symbol.clone(),
        vec![OrderBookLevel::new(Decimal::new(10001, 4), Decimal::from(1000))],
        vec![OrderBookLevel::new(Decimal::new(10003, 4), Decimal::from(1000))],
    );
...
async fn test_multiple_stablecoins() -> Result<()> {
    ...
    let usdt_book = OrderBook::new(
        ExchangeId::Kraken,
        usdt_symbol.clone(),
        vec![OrderBookLevel::new(Decimal::new(1003, 3), Decimal::from(2000))],
        vec![OrderBookLevel::new(Decimal::new(1005, 3), Decimal::from(2000))],
    );
    ...
    let dai_book = OrderBook::new(
        ExchangeId::Bitstamp,
        dai_symbol.clone(),
        vec![OrderBookLevel::new(Decimal::new(996, 3), Decimal::from(1500))],
        vec![OrderBookLevel::new(Decimal::new(998, 3), Decimal::from(1500))],
    );
    ...
    println!("Detected {} signals across multiple stablecoins", signals.len());
    
    // Should detect opportunities for both stablecoins
    let usdt_signals: Vec<_> = signals.iter()
        .filter(|s| s.symbol.base == "USDT")
        .collect();
    let dai_signals: Vec<_> = signals.iter()
        .filter(|s| s.symbol.base == "DAI")
        .collect();
    
    println!("USDT signals: {}, DAI signals: {}", usdt_signals.len(), dai_signals.len());
