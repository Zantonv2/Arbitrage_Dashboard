...
use arbitrage_core::{
    storage::{ExecutionStatus, SignalQuery, SignalStatus, StorageConfig, StorageService},
    types::{ExchangeId, ExecutionInstruction, Order, OrderType, Side, Signal, Symbol},
};
...
async fn test_signal_storage_and_retrieval() {
    ...
    // Store signal
    storage
        .store_signal(signal.clone(), confidence_score, SignalStatus::Detected)
        .await
        .expect("Failed to store signal");

    // Query signals
    let query = SignalQuery::default();
    let stored_signals = storage
        .query_signals(&query)
        .await
...
async fn test_execution_storage_and_retrieval() {
    ...
    // Store signal first
    storage
        .store_signal(signal.clone(), Decimal::new(85, 2), SignalStatus::Detected)
        .await
        .expect("Failed to store signal");

    // Store execution
    storage
        .store_execution(execution.clone(), ExecutionStatus::Pending)
        .await
        .expect("Failed to store execution");

    // Retrieve execution
    let stored_execution = storage
        .get_execution(signal.id)
        .await
...
async fn test_signal_status_updates() {
    ...
    // Update status
    storage
        .update_signal_status(signal.id, SignalStatus::Executed)
        .await
...
async fn test_signal_filtering() {
    ...
    storage
        .store_signal(signal1.clone(), Decimal::new(90, 2), SignalStatus::Detected)
        .await
        .expect("Failed to store signal1");

    storage
        .store_signal(signal2.clone(), Decimal::new(75, 2), SignalStatus::Filtered)
        .await
        .expect("Failed to store signal2");

    storage
        .store_signal(signal3.clone(), Decimal::new(95, 2), SignalStatus::Executed)
        .await
    ...
    let high_confidence_signals = storage
        .query_signals(&query)
        .await
        .expect("Failed to query");
...
async fn test_storage_statistics() {
    ...
    for i in 0..5 {
        ...
        storage
            .store_signal(signal.clone(), Decimal::new(85, 2), status)
            .await
            .expect("Failed to store signal");
        ...
        if i < 2 {
            let execution = create_test_execution(signal.id);
            storage
                .store_execution(execution, ExecutionStatus::Pending)
                .await
                .expect("Failed to store execution");
        }
    }
    ...
    // Check status counts
    assert_eq!(
        stats.signal_status_counts.get(&SignalStatus::Detected),
        Some(&3)
    );
    assert_eq!(
        stats.signal_status_counts.get(&SignalStatus::Executed),
        Some(&1)
    );
    assert_eq!(
        stats.signal_status_counts.get(&SignalStatus::Filtered),
        Some(&1)
    );

    assert_eq!(
        stats.execution_status_counts.get(&ExecutionStatus::Pending),
        Some(&2)
    );
}
...
async fn test_concurrent_storage_operations() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            storage_clone
                .store_signal(signal, Decimal::new(85, 2), SignalStatus::Detected)
                .await
        });
        handles.push(handle);
    }
...
async fn test_error_handling() {
    ...
    let result = StorageService::new(config).await;
...
async fn test_decimal_precision() {
    ...
    assert_eq!(
        stored_signal.signal.gross_profit_percent,
