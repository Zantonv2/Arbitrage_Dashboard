...
use crate::connector::{ExchangeConnector, ConnectorStats, HealthStatus};
use crate::events::{ConnectionEvent, MarketDataEvent, EventStats};
...
impl Default for ExchangeManagerConfig {
    fn default() -> Self {
        ...
        rate_limits.insert(ExchangeId::OKX, RateLimitConfig {
            requests_per_second: 40,
            burst_capacity: 80,
            window_seconds: 60,
        });
        
        rate_limits.insert(ExchangeId::ByBit, RateLimitConfig {
            requests_per_second: 60,
            burst_capacity: 120,
            window_seconds: 60,
        });
        
        rate_limits.insert(ExchangeId::MEXC, RateLimitConfig {
            requests_per_second: 20,
            burst_capacity: 40,
            window_seconds: 60,
        });
        
        rate_limits.insert(ExchangeId::GateIo, RateLimitConfig {
            requests_per_second: 30,
            burst_capacity: 60,
            window_seconds: 60,
        });
        
        rate_limits.insert(ExchangeId::Bitstamp, RateLimitConfig {
            requests_per_second: 8000,
            burst_capacity: 16000,
            window_seconds: 60,
        });
        
        rate_limits.insert(ExchangeId::Kraken, RateLimitConfig {
...
impl ExchangeManager {
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        info!("Initializing exchange manager with {} exchanges", 
        ...
        for exchange_id in &self.config.enabled_exchanges {
            ...
            let rate_limit_config = self.config.rate_limits
                .get(exchange_id)
                .cloned()
                .unwrap_or_default();
        ...
        for (exchange_id, connector) in connectors.iter_mut() {
            ...
            match connector.subscribe_symbols(symbols).await {
                Ok(()) => {
                    ...
                    let _ = self.event_sender.send(ConnectionEvent::SubscriptionConfirmed {
                        exchange,
    ...
    pub async fn unsubscribe_symbols(&mut self, symbols: &[Symbol]) -> Result<()> {
        info!("Unsubscribing from {} symbols across all exchanges", symbols.len());
    ...
    pub async fn get_order_books_all(&self, symbol: &Symbol) -> HashMap<ExchangeId, Result<OrderBook>> {
        let connectors = self.connectors.read().await;
        ...
        for (exchange_id, connector) in connectors.iter() {
            ...
            // Apply rate limiting
            let rate_limit_result = if let Some(rate_limiter) = self.rate_limiters.read().await.get(&exchange) {
        ...
        for (exchange_id, connector) in connectors.iter() {
            match connector.health_check().await {
                ...
                Err(e) => {
                    ...
                    status_map.insert(*exchange_id, HealthStatus {
                        is_connected: false,
